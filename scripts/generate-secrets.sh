...
#
# - Local-only secrets (passwords, runner secret) are randomly generated.
# - Google OAuth2 credentials are written as placeholders for manual replacement.
# - Forgejo API tokens are written as placeholders (generated later by setup.sh).
#
# Usage:
...
echo -n "$(generate_password)" > "${SECRETS_DIR}/forgejo_admin_password"
echo -n "$(generate_password)" > "${SECRETS_DIR}/forgejo_agent_password"
echo -n "$(openssl rand -hex 20)" > "${SECRETS_DIR}/forgejo_runner_secret"

# --- Copy postgres password to Spring datasource password ---

echo -n "${POSTGRES_PASSWORD}" > "${SECRETS_DIR}/spring.datasource.password"

# --- Write placeholder Forgejo API tokens ---

echo -n "GENERATED_BY_SETUP_SH" > "${SECRETS_DIR}/forgejo_auth_token"
echo -n "GENERATED_BY_SETUP_SH" > "${SECRETS_DIR}/forgejo_pr_agent_token"
...
echo "  ${SECRETS_DIR}/forgejo_admin_password"
echo "  ${SECRETS_DIR}/forgejo_agent_password"
echo "  ${SECRETS_DIR}/forgejo_runner_secret"
echo "  ${SECRETS_DIR}/spring.datasource.password (copied from postgres_password)"
echo ""
echo "Placeholder tokens (will be generated by setup.sh):"
echo "  ${SECRETS_DIR}/forgejo_auth_token"
echo "  ${SECRETS_DIR}/forgejo_pr_agent_token"
echo ""
