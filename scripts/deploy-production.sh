#!/bin/bash
#
# Deploy and verify the Skiploom production stack.
#
# Rebuilds Docker images from source, restarts production services, and verifies
...
SECRETS_DIR="secrets"
BACKEND_URL="http://localhost:8082/api/health"
...
docker compose --profile production up -d --wait --build
...
if ! docker compose --profile production ps; then
