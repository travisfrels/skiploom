...
| Low | Investigate whether the review-pr skill can detect and prevent duplicate reviews on the same PR. **Resolved:** Duplicate was caused by manual re-trigger of the `/review-pr` skill (40-second gap, identical content, same commit). No tooling change needed — no duplicate-detection guard warranted for a manual invocation. | [#142](https://github.com/travisfrels/skiploom/issues/142) |
